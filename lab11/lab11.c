...
int main() {
  ...
  if (pubkey == NULL) {
    handle_error("Failed to parse the PEM public key");
  }
...
#define MAX_FILE_SIZE 512
  ...
  if (mdctx == NULL) {
    handle_error("Failed to create EVP_MD_CTX");
  }

  if (EVP_DigestVerifyInit(mdctx, NULL, EVP_sha256(), NULL, pubkey) <= 0) {
    EVP_MD_CTX_free(mdctx);
    handle_error("EVP_DigestVerifyUpdate failed");
  }
  ...
  int auth_status = EVP_DigestVerifyFinal(mdctx, signature, sig_len);
  EVP_MD_CTX_free(mdctx);
  return auth_status;
}
