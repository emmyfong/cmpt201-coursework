...
Client uses TCP protocol and utilizes the `socket()` and `connect()` system calls to create a stream

2. What data is the client sending to the server?
Sends fixed predefined string messages ("Hello", "Apple", "Car", "Green", "Dog")

Understanding the Server:
1. Explain the argument that the `run_acceptor` thread is passed as an argument.
Passed by pointer to a acceptor_args which contains shared boolean flag `run` to control thread loop
and pointers to the shared linked list and its mutex lock

2. How are recieved messages stored?
Dynamically allocated and stored in heap-allocated linked list `list_node`

3. What does `main()` do with the received messages?
It waits using a loop until all the expected messages are recieved then calls `collect_all()` to
print the messages, count them, and free allocated memory

4. How are threads used in this sample code?
One main thread and one acceptor thread that listens for the incoming connections and multiple
client threads created by acceptor to read data concurrently

Explain the use of non-blocking sockets in this lab:
1. How are sockets made non-blocking?
By using `fcntl()` to get current file descirptor flag and then setting them again with `O_NONBLOCK`
flag added using bitwise or

2. What sockets are made non-blocking?
Server's main listening socket and ever individual client connection socket

3. Why are these sockets made non-blocking? What purpose does it serve?
By default `accept()` and `read()` block the thread indefinitely until a connection is made or data
is received. Since they are non-blocking, they can return an error if there is nothing to process,
preventing the threads from freezing and allowing them to loop back and check if their `run` flag is
set to false
